<template>
	<view class="blogitem">
		...
		<view class="info">
			<view class="box"><text class="iconfont icon-eye"></text> <text>{{ item.view_count }}</text></view>
		...
		<u-action-sheet  :actions="list"  cancelText="取消" :show="sheetShow" :closeOnClickOverlay="true" :closeOnClickAction="true" @select="sheetSelect" @close="onClose"></u-action-sheet>
	</view>
</template>

<script>
	import pageJson from '@/pages.json'
	import { getNickname, getAvatar, likeFun }  from '../../utils/tools.js'
	...
	export default {
		...
		methods: {
			...
			clickMore() {
				...
				if(uid == this.item.user_id[0]._id || this.uniDHasRole('admin') || this.uniDHasRole('webmaster')) {
