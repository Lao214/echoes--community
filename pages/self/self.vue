			...
			<view class="list">
				<view class="group">					
					...
					<view class="item" @click="myLike">
						...
						<view class="right"><text class="iconfont icon-icon-test"></text></view>
					</view>
					<view class="item">
	...
	export default {
		...
		methods: {
			...
			async getTotal() {
				...
				let likeCount = await db.collection("quanzi_article").where(`user_id == $cloudEnv_uid`).groupBy('user_id').groupField('sum(like_count) as totalLike').get()
