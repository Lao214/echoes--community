<template>
	<view class="edit">
		...
		<view class="content">
			<editor class="myEdit" placeholder="请写点东西吧" show-img-size show-img-toolbar show-img-resize @ready="onEditReady" @focus="onFocus" @statuschange="onStatuschange">
		...
		<view class="tools" v-if="toolShow">
			...
			<view class="item" @click="clickInsertImg">
				...
				uni.chooseImage({
					success:async res => {
						...
						for(let item of res.tempFiles) {
							...
							let randomName = Date.now() + "" + String(Math.random()).substr(3,6)  + fileName
